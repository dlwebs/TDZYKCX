function varargout = Land_Resource_Sustainability_Management_System(varargin)
% LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM MATLAB code for Land_Resource_Sustainability_Management_System.fig
%      LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM, by itself, creates a new LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM or raises the existing
%      singleton*.
%
%      H = LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM returns the handle to a new LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM or the handle to
%      the existing singleton*.
%
%      LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM.M with the given input arguments.
%
%      LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM('Property','Value',...) creates a new LAND_RESOURCE_SUSTAINABILITY_MANAGEMENT_SYSTEM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Land_Resource_Sustainability_Management_System_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Land_Resource_Sustainability_Management_System_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Land_Resource_Sustainability_Management_System

% Last Modified by GUIDE v2.5 23-Apr-2024 17:07:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Land_Resource_Sustainability_Management_System_OpeningFcn, ...
                   'gui_OutputFcn',  @Land_Resource_Sustainability_Management_System_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Land_Resource_Sustainability_Management_System is made visible.
function Land_Resource_Sustainability_Management_System_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Land_Resource_Sustainability_Management_System (see VARARGIN)

% Choose default command line output for Land_Resource_Sustainability_Management_System
...
function varargout = Land_Resource_Sustainability_Management_System_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text26, 'String', '  土地资源可持续发展综合管理系统');

handles.uipanel2.Title = '土地使用管理';
handles.uipanel3.Title = '土地质量监测管理';
handles.uipanel4.Title = '土地使用计划设置';
handles.uipanel5.Title = '土地规划政策管理';
handles.uipanel6.Title = '土地开发利用评估';

set(handles.pushbutton8, 'String', '确认选择');
set(handles.pushbutton9, 'String', '保存信息');
set(handles.pushbutton10, 'String', '确认设置');
set(handles.pushbutton11, 'String', '保存信息');
set(handles.pushbutton12, 'String', '利用评估');

set(handles.text_btn_text_1, 'String', '导入土地资源可持续发展工艺流程图');
set(handles.text_btn_text_2, 'String', '绘制土地资源发展效果分析三维图');
set(handles.text_btn_text_3, 'String', '绘制土地开发利用分析柱状图');
unit_content = {'', '', '', '', '分'};

text_contents1 = {'城市建设用地', '农业用地', '林地', '草原', '湿地', '保护区', '未利用地'};											
text_contents2 = {'土壤pH值', '有机质含量', '重金属污染', '盐碱化程度', '土地退化', '生物多样性', '地表覆盖'};											
text_contents3 = {'土地再分类', '土地整治', '土地开发', '土地批准', '土地利用监测', '环境影响评估', '土地使用报告'};											
text_contents4 = {'城市扩展限制', '农地保护区域', '林地补植要求', '水源保护区', '生态红线区', '土地复垦指标', '土地使用效率'};											
text_contents5 = {'开发潜力评估', '可持续性评估', '生态影响评估', '经济效益评估', '社会影响评估', '资源消耗评估', '生物多样性影响'};											
...
features2 = [
    struct('type', 'popupmenu', 'items', {formatItems('6.5|7.0|7.5')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('高|中|低')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('铅|汞|镉')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('轻度|中度|重度')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('水土流失|沙化|压实')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('高|中|低')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('完整|破碎|裸露')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
];										
											
features3 = [
    struct('type', 'popupmenu', 'items', {formatItems('农地|商业地|保护区')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('地形调整|土壤改良|排水改善')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('商业开发|住宅开发|工业开发')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('政府批准|私人批准|外资批准')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('卫星监测|地面监测|无人机监测')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('低影响|中等影响|高影响')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
    struct('type', 'popupmenu', 'items', {formatItems('自动生成|手动提交')}, 'width', 100, 'fontSize', 10, 'range', [], 'defaultValue', '', 'unit', '', 'unitFontSize', 14),...
];										
											
features4 = [
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', 'km', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '%', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', 'ha', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', 'km²', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '%', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', 'ha', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
    struct('type', 'edit', 'fontSize', 12, 'width', 60, 'unit', '%', 'unitFontSize', 14, 'defaultValue', '', 'items', {{}}, 'range', []),...
...
handles = createUIComponents(handles, handles.uipanel6, features5, 39, 200, [255, 255, 255], [0, 0, 0]);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.text40,'String','设置完成');
pause(1); 
...
% handles    structure with handles and user data (see GUIDATA)
axes(handles.axes2);
x_data = [1, 2, 3, 4, 5]; % 土地利用类型的数据，例如1-商业用地，2-农业用地等
y_data = [10, 20, 30, 40, 50]; % 土地开发强度的数据
z_data = [75, 80, 85, 90, 95]; % 土地保护成效的数据

% 创建三维散点图
scatter3(handles.axes2, x_data, y_data, z_data, 'filled');
title(handles.axes2, '土地资源发展效果分析三维图');
xlabel(handles.axes2, '土地利用类型');
ylabel(handles.axes2, '土地开发强度');
zlabel(handles.axes2, '土地保护成效');

% 设置图形属性
grid(handles.axes2, 'on');  % 开启网格
colormap(handles.axes2, 'jet'); % 设置色彩映射
colorbar(handles.axes2);        % 显示颜色条
drawnow; pause(1); 
...
set(gca,'XTickLabel',{'开发潜力评估', '可持续性评估', '生态影响评估', '经济效益评估', '社会影响评估', '资源消耗评估', '生物多样性影响'});
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
h = msgbox('土地资源可持续发展综合管理系统为用户提供了土地使用管理、土地质量监测管理、土地使用计划设置、土地规划政策管理、土地开发利用评估等功能。', '关于本软件','help');
jMsgBox = h.JavaFrame.getFigurePanelContainer.getComponent(0).getTopLevelAncestor;
jMsgBox.setAlwaysOnTop(true);
figure(handles.figure1);
drawnow; pause(1); 
...
% UIWAIT makes Land_Resource_Sustainability_Management_System wait for user response (see UIRESUME)
...
function setImageToAxesWithAlpha(handles, imageName, axesTag, backgroundColor)
    % 构建图片路径 , 'Land_Resource_Sustainability_Management_System'  Administrati
    if isdeployed
